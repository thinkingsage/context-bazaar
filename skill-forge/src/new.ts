...
import { AssetTypeSchema, type AssetType } from "./schemas";
...
export async function newCommand(artifactName: string, options: NewCommandOptions = {}): Promise<void> {
  const knowledgeDir = "knowledge";
  const artifactDir = join(knowledgeDir, artifactName);

  if (await exists(artifactDir)) {
    console.error(chalk.red(`Error: Artifact "${artifactName}" already exists at ${artifactDir}. Choose a different name.`));
    process.exit(1);
  }
  ...
  if (options.type) {
    ...
    if (!parsed.success) {
      ...
      console.error(chalk.red(`Error: Unknown asset type "${options.type}". Valid types: ${valid}`));
      process.exit(1);
    }
  ...
  if (options.yes) {
    console.error(`\nNext steps:`);
    console.error(`  1. Edit ${join(artifactDir, "knowledge.md")} with your content`);
    console.error(`  2. Add hooks to ${join(artifactDir, "hooks.yaml")} (optional)`);
    console.error(`  3. Add MCP servers to ${join(artifactDir, "mcp-servers.yaml")} (optional)`);
