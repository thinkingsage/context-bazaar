...
import { loadCollections, validateArtifactCollectionRefs, buildCollectionMembership } from "./collections";
...
async function buildCollectionForHarness(
  ...
  const collectionOutDir = join(distDir, harness, "collections", collection.name);
...
export async function buildCollections(options: {
  ...
  if (collections.length === 0) {
    console.error(chalk.yellow("No collection manifests found in collections/"));
    return results;
  }
  ...
  const refWarnings = validateArtifactCollectionRefs(catalogEntries, collections);
  ...
  const targetHarnesses: HarnessName[] = harness ? [harness] : [...SUPPORTED_HARNESSES];

  for (const collection of collections) {
    const members = membership.get(collection.name) ?? [];

    if (members.length === 0) {
      console.error(chalk.yellow(`  ⚠ Collection "${collection.name}" has no member artifacts`));
    }

    for (const h of targetHarnesses) {
      const { artifactsIncluded, filesWritten } = await buildCollectionForHarness(
        collection,
        members,
        h,
        distDir,
...
export async function collectionStatusCommand(): Promise<void> {
  ...
  const nextArg = collectionArgIdx >= 0 ? process.argv[collectionArgIdx + 1] : undefined;
  if (nextArg === "help") {
  ...
  if (collections.length === 0 && warnings.length === 0) {
    ...
    console.log("");
    console.log(chalk.dim("  Collections are curated bundles of artifacts (e.g. \"aws\", \"security\", \"onboarding\")."));
    console.log(chalk.dim("  Artifacts declare membership in their frontmatter: ") + chalk.cyan("collections: [aws]"));
    console.log("");
    ...
    if (p.isCancel(create) || !create) {
      console.log("");
      console.log(chalk.dim("  Run ") + chalk.cyan("forge collection new <name>") + chalk.dim(" when you're ready."));
      console.log("");
      return;
    }
    ...
    const name = await p.text({
      ...
      validate: (v) => {
        ...
        if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(v.trim())) return "Must be kebab-case (e.g. my-collection)";
      },
    });
  ...
  const totalMembers = [...membership.values()].reduce((s, m) => s + m.length, 0);

  console.log(
    chalk.bold(`\n  Collections`) +
    chalk.dim(` (${total} manifest${total !== 1 ? "s" : ""}, ${totalMembers} total memberships)\n`),
  );

  if (collections.length > 0) {
    // Column widths
    const nameWidth = Math.max(...collections.map((c) => c.displayName.length), 4) + 2;
    const trustWidth = 12;
    ...
    for (const c of collections) {
      ...
      if (members.length > 0 && members.length <= 5) {
        ...
        console.log(chalk.dim(`    ${members.slice(0, 4).join(", ")}, +${members.length - 4} more`));
      }
    }
  }
  ...
  if (warnings.length > 0) {
    const uniqueUnknown = [...new Set(
      warnings.map((w) => w.message.match(/"([^"]+)"/)?.[1] ?? "?"),
    )];
    console.log("");
    for (const name of uniqueUnknown) {
      console.log(chalk.yellow(`  ⚠ Artifacts declare collection "${name}" — no manifest found in collections/`));
    }
  }

  // Artifacts with no collection membership
  const unaffiliated = catalogEntries.filter((e) => e.collections.length === 0).length;
  if (unaffiliated > 0) {
    console.log(chalk.dim(`\n  ${unaffiliated} artifact${unaffiliated !== 1 ? "s" : ""} not in any collection`));
  }

  console.log("");
  console.log(chalk.dim("  forge collection build   — generate distributable bundles"));
  console.log("");
}
...
export async function collectionNewCommand(name?: string): Promise<void> {
  ...
  const collectionName = name ?? await p.text({
    message: "Collection name (kebab-case)",
    placeholder: "my-collection",
    validate: (v) => {
      if (!v || v.trim().length === 0) return "Name is required";
  ...
  if (p.isCancel(collectionName)) { p.cancel("Cancelled."); return; }

  const filePath = join(collectionsDir, `${collectionName}.yaml`);
  if (await exists(filePath)) {
    console.error(chalk.red(`  Collection "${collectionName}" already exists at ${filePath}`));
    process.exit(1);
  }

  const displayName = await p.text({
    message: "Display name",
    placeholder: collectionName.split("-").map((w) => w[0].toUpperCase() + w.slice(1)).join(" "),
  }) as string;
  if (p.isCancel(displayName)) { p.cancel("Cancelled."); return; }
  ...
  const trust = await p.select({
    ...
    options: TrustLaneSchema.options.map((t) => ({
      value: t,
      label: t,
      hint: { official: "maintained by core team", partner: "maintained by trusted partner", community: "community contributed", experimental: "experimental / untested" }[t],
    })),
    initialValue: "community" as const,
  });
  if (p.isCancel(trust)) { p.cancel("Cancelled."); return; }

  const resolvedDisplayName = (displayName as string).trim() ||
    collectionName.split("-").map((w) => w[0].toUpperCase() + w.slice(1)).join(" ");
...
export async function collectionBuildCommand(options: { harness?: string }): Promise<void> {
  const distDir = "dist";
  const collectionsDir = "collections";

  if (!(await exists(distDir))) {
    console.error(chalk.red("No dist/ directory found. Run `forge build` first."));
    process.exit(1);
  }

  if (options.harness && !(SUPPORTED_HARNESSES as readonly string[]).includes(options.harness)) {
