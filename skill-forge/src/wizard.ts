...
export async function promptFrontmatter(
  ...
  if (preSelectedType) {
    ...
    const typeOptions = (Object.keys(ASSET_TYPE_DESCRIPTIONS) as AssetType[]).map((t) => ({
      value: t,
      label: t,
  ...
  const inclusion = await p.select({
    ...
    options: [
      { value: "always" as const, label: "always", hint: "Included in every AI session automatically" },
      { value: "fileMatch" as const, label: "fileMatch", hint: "Included only when matching files are open" },
      { value: "manual" as const, label: "manual", hint: "Included only when explicitly referenced" },
    ],
  });
    ...
    const filePatternsRaw = await p.text({
      ...
      validate: (val) => {
        ...
        return validateField(
          FrontmatterSchema.shape.file_patterns,
          parsed,
        );
      },
    });
  ...
  for (const harness of selectedHarnesses) {
    const registryEntry = HARNESS_FORMAT_REGISTRY[harness as keyof typeof HARNESS_FORMAT_REGISTRY];
  ...
  const ecosystemRaw = await p.text({
    ...
    validate: (val) => {
      ...
      const parsed = parseCommaSeparated(val);
      return validateField(
        FrontmatterSchema.shape.ecosystem,
        parsed,
      );
    },
  });
  ...
  const frontmatter: Frontmatter = {
    ...
    ecosystem: ecosystemRaw
      ? parseCommaSeparated(ecosystemRaw as string)
      : [],
    depends: [],
    enhances: [],
...
async function promptSingleHook(): Promise<CanonicalHook | null> {
  ...
  const actionType = await p.select({
    ...
    options: [
      { value: "ask_agent" as const, label: "ask_agent", hint: "Send a prompt to the AI agent" },
      { value: "run_command" as const, label: "run_command", hint: "Execute a shell command" },
    ],
  });
...
export async function runWizard(
  ...
  const frontmatter = await promptFrontmatter(artifactName, displayName, preSelectedType);
