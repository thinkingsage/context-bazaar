...
import { cp, exists, mkdir, mkdtemp, readFile, rm } from "node:fs/promises";
...
describe("forge new", () => {
	...
	test("error when artifact directory already exists", async () => {
		...
		await mkdir(join(tempDir, "knowledge", "existing-artifact"), {
			recursive: true,
		});
...
describe("CLI flag routing", () => {
	...
	test("existing directory error message mentions artifact name", async () => {
		await mkdir(join(tempDir, "knowledge", "duplicate-name"), {
			recursive: true,
		});
