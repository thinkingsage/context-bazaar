...
const { buildTutorialSteps, showProgress, resolveArtifactName, showWelcome, showCompletion, TUTORIAL_DEFAULTS } =
...
describe("Tutorial unit tests", () => {
  ...
  test("buildTutorialSteps embeds artifact name in explanations", () => {
    ...
    const hasArtifactName = steps.some((s) => s.explanation.includes("my-cool-artifact"));
  ...
  test("resolveArtifactName with existing dir and overwrite returns original name", async () => {
    ...
    try {
      ...
      await mkdir(join(tempDir, "knowledge", "hello-world"), { recursive: true });
      process.chdir(tempDir);
