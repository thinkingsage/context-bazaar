...
const EVAL_CONTEXTS_DIR = resolve(import.meta.dir, "../../templates/eval-contexts");
...
describe("Prompt resolver", () => {
  ...
  test("handles mixed file:// and inline prompts", () => {
    ...
    expect(prompts[0].startsWith("file:///") || prompts[0].startsWith("file://C:")).toBe(true);
    expect(prompts[1]).toBe("Inline prompt");
    expect(prompts[2].startsWith("file:///") || prompts[2].startsWith("file://C:")).toBe(true);
  });
});
...
describe("Harness context wrapping", () => {
  ...
  test("returns prompt as-is when template does not exist", () => {
    ...
    const emptyTemplateEnv = createTemplateEnv(join(tempDir, "empty-templates"));
