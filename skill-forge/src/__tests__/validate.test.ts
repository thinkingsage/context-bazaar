...
import { validateArtifact, validateAll } from "../validate";
import { CATEGORIES, FrontmatterSchema, type Category } from "../schemas";
...
`,
    ...
    const hookError = result.errors.find((e) => e.filePath.includes("hooks.yaml"));
...
Body content.`,
    ...
    expect(result.valid).toBe(false);
    expect(result.errors.length).toBeGreaterThan(0);
    const harnessError = result.errors.find((e) => e.field.startsWith("harnesses"));
...
Body content.`,
    ...
    const typeWarnings = result.warnings?.filter((w) => w.field === "type") ?? [];
...
Body content.`,
    ...
    const result = await validateArtifact(artifactDir);
    expect(result.valid).toBe(true);
    expect((result.warnings?.filter((w) => w.field === "type") ?? []).length).toBe(0);
  });
...
describe("Catalog metadata evolution - validation extensions", () => {
  ...
  test("duplicate ecosystem values preserved", () => {
    ...
    expect(result.success).toBe(true);
    if (!result.success) return;
    expect(result.data.ecosystem).toEqual(["typescript", "typescript", "react"]);
  });
  ...
  test("schema does not enforce reference existence at parse time", () => {
    ...
    expect(result.success).toBe(true);
    if (!result.success) return;
    expect(result.data.depends).toEqual(["nonexistent-artifact", "another-missing"]);
...
Body content.`,
    ...
    const dependsWarning = resultA!.warnings!.find((w) => w.field === "depends");
    expect(dependsWarning).toBeDefined();
    expect(dependsWarning!.message).toContain("nonexistent-dep");

    const enhancesWarning = resultA!.warnings!.find((w) => w.field === "enhances");
