...
describe("cursorAdapter", () => {
  ...
  test("maps always inclusion to always", () => {
    const artifact = makeArtifact({ frontmatter: makeFrontmatter({ inclusion: "always" }) });
    const result = cursorAdapter(artifact, templateEnv);

    const ruleFile = result.files.find((f) => f.relativePath === ".cursor/rules/test-artifact.md");
    expect(ruleFile).toBeDefined();
    expect(ruleFile!.content).toContain("inclusion: always");
  });

  test("maps fileMatch inclusion to auto", () => {
    const artifact = makeArtifact({ frontmatter: makeFrontmatter({ inclusion: "fileMatch" }) });
    const result = cursorAdapter(artifact, templateEnv);
  ...
  test("maps manual inclusion to agent-requested", () => {
    const artifact = makeArtifact({ frontmatter: makeFrontmatter({ inclusion: "manual" }) });
    const result = cursorAdapter(artifact, templateEnv);
  ...
  test("generates .cursor/mcp.json from MCP servers", () => {
    const artifact = makeArtifact({
      mcpServers: [
        { name: "my-server", command: "uvx", args: ["my-server@latest"], env: { API_KEY: "secret" } },
      ],
    });

    const result = cursorAdapter(artifact, templateEnv);
    const mcpFile = result.files.find((f) => f.relativePath === ".cursor/mcp.json");
  ...
  test("skips hooks with warning", () => {
    const artifact = makeArtifact({
      hooks: [makeHook({ name: "Lint Hook", event: "file_edited", action: { type: "run_command", command: "npm run lint" } })],
    });
...
describe("windsurfAdapter", () => {
  ...
  test("generates rule file with artifact body", () => {
    ...
    const ruleFile = result.files.find((f) => f.relativePath === ".windsurf/rules/test-artifact.md");
    expect(ruleFile).toBeDefined();
    expect(ruleFile!.content).toContain("This is test content.");
  });
  ...
  test("copies workflows to .windsurf/workflows/", () => {
    ...
    const result = windsurfAdapter(artifact, templateEnv);
    const deployFile = result.files.find((f) => f.relativePath === ".windsurf/workflows/deploy.md");
    const setupFile = result.files.find((f) => f.relativePath === ".windsurf/workflows/setup.md");
  ...
  test("no workflow files when artifact has no workflows", () => {
    ...
    const result = windsurfAdapter(artifact, templateEnv);
    const wfFiles = result.files.filter((f) => f.relativePath.includes("workflows/"));
  ...
  test("generates .windsurf/mcp.json from MCP servers", () => {
    const artifact = makeArtifact({
      mcpServers: [
        { name: "docs-server", command: "npx", args: ["docs-server"], env: { PORT: "3000" } },
      ],
    });

    const result = windsurfAdapter(artifact, templateEnv);
    const mcpFile = result.files.find((f) => f.relativePath === ".windsurf/mcp.json");
    expect(mcpFile).toBeDefined();
...
describe("clineAdapter", () => {
  ...
  test("generates rule file with artifact body", () => {
    ...
    const ruleFile = result.files.find((f) => f.relativePath === ".clinerules/test-artifact.md");
    expect(ruleFile).toBeDefined();
    expect(ruleFile!.content).toContain("This is test content.");
  });
  ...
  test("generates executable hook scripts for run_command actions", () => {
    ...
    const result = clineAdapter(artifact, templateEnv);
    const hookFile = result.files.find((f) => f.relativePath.includes("hooks/"));
  ...
  test("generates multiple hook scripts", () => {
    const artifact = makeArtifact({
      hooks: [
        makeHook({ name: "Lint", event: "file_edited", action: { type: "run_command", command: "npm run lint" } }),
        makeHook({ name: "Test", event: "agent_stop", action: { type: "run_command", command: "npm test" } }),
      ],
    });

    const result = clineAdapter(artifact, templateEnv);
    const hookFiles = result.files.filter((f) => f.relativePath.includes("hooks/"));
  ...
  test("generates MCP configuration from MCP servers", () => {
    const artifact = makeArtifact({
      mcpServers: [
        { name: "my-mcp", command: "node", args: ["server.js"], env: { DEBUG: "true" } },
      ],
    });

    const result = clineAdapter(artifact, templateEnv);
    const mcpFile = result.files.find((f) => f.relativePath === ".clinerules/mcp.json");
    expect(mcpFile).toBeDefined();
  ...
  test("preserves command string fidelity in hook scripts", () => {
    ...
    const artifact = makeArtifact({
      hooks: [makeHook({ name: "Complex Cmd", event: "agent_stop", action: { type: "run_command", command } })],
    });
...
describe("qdeveloperAdapter", () => {
  ...
  test("generates rule file with artifact body", () => {
    ...
    const ruleFile = result.files.find((f) => f.relativePath === ".q/rules/test-artifact.md");
    expect(ruleFile).toBeDefined();
    expect(ruleFile!.content).toContain("This is test content.");
  });
  ...
  test("generates agent files from workflows", () => {
    ...
    const result = qdeveloperAdapter(artifact, templateEnv);
    const deployAgent = result.files.find((f) => f.relativePath === ".q/agents/deploy.md");
    const setupAgent = result.files.find((f) => f.relativePath === ".q/agents/setup.md");
  ...
  test("no agent files when artifact has no workflows", () => {
    const artifact = makeArtifact({ workflows: [] });
    const result = qdeveloperAdapter(artifact, templateEnv);
    const agentFiles = result.files.filter((f) => f.relativePath.includes("agents/"));
  ...
  test("generates MCP configuration from MCP servers", () => {
    const artifact = makeArtifact({
      mcpServers: [
        { name: "q-server", command: "python", args: ["-m", "q_server"], env: { REGION: "us-east-1" } },
      ],
    });
  ...
  test("skips hooks with warning", () => {
    const artifact = makeArtifact({
      hooks: [makeHook({ name: "Some Hook", event: "agent_stop", action: { type: "run_command", command: "echo hi" } })],
    });
