...
function optionsOf(call: { fn: string; args: unknown[] }): unknown[] {
  return ((call.args[0] as Record<string, unknown>)?.options as unknown[]) ?? [];
}
...
describe("Wizard per-harness format prompting", () => {
  ...
  test("harness multi-select shows descriptive labels", async () => {
    ...
    const options = optionsOf(harnessCall!) as { value: string; label: string }[];
    // Check that labels contain descriptions
    const kiroOption = options.find((o) => o.value === "kiro");
    expect(kiroOption?.label).toContain("Steering files or powers for Kiro IDE");

    const copilotOption = options.find((o) => o.value === "copilot");
    expect(copilotOption?.label).toContain("Instructions or agents for GitHub Copilot");
    ...
    expect(qdevOption?.label).toContain("Rules or agents for Amazon Q Developer");
  });
  ...
  test("non-default format selections are written to harness-config", async () => {
    ...
    const harnessConfigField = (fm as Record<string, unknown>)["harness-config"] as
  ...
  test("format prompt shows valid options with descriptions", async () => {
    ...
    const options = optionsOf(kiroFormatCall!) as { value: string; label: string; hint: string }[];
