...
const safeString = () =>
  fc.string({ minLength: 1, maxLength: 30 }).filter((s) => s.length > 0 && !s.includes("\0") && !s.includes("\n") && s.trim() === s);
    ...
    fc.assert(
      fc.property(canonicalHookArb, (hook) => {
        ...
        if (hook.action.type === "ask_agent") {
          expect((result.data.action as { prompt: string }).prompt).toBe(hook.action.prompt);
        } else {
          expect((result.data.action as { command: string }).command).toBe(hook.action.command);
        }
        expect(result.data.description).toBe(hook.description);

        // Condition comparison
        if (hook.condition) {
          expect(result.data.condition?.file_patterns).toEqual(hook.condition.file_patterns);
          expect(result.data.condition?.tool_types).toEqual(hook.condition.tool_types);
        } else {
...
describe("Catalog metadata evolution properties", () => {
  ...
  test("Property 5: Backward compatibility — legacy frontmatter parses with defaults", () => {
    ...
    const legacyFrontmatterArb = fc.record({
      ...
      version: fc.tuple(fc.nat(9), fc.nat(9), fc.nat(9)).map(([a, b, c]) => `${a}.${b}.${c}`),
      harnesses: fc.uniqueArray(harnessNameArb, { minLength: 1, maxLength: 7 }),
      type: fc.constantFrom("skill", "power", "rule") as fc.Arbitrary<Frontmatter["type"]>,
      inclusion: fc.constantFrom("always", "fileMatch", "manual") as fc.Arbitrary<Frontmatter["inclusion"]>,
      file_patterns: fc.option(fc.array(legacySafeString(), { maxLength: 3 }), { nil: undefined }),
    });
