import { describe, test, expect, spyOn, beforeEach, afterEach, beforeAll, afterAll } from "bun:test";
import { validatePort, escapeHtml, handleRequest, generateHtmlPage } from "../browse";
import { makeCatalogEntry } from "./test-helpers";

// --- Mock catalog data for handleRequest tests ---
const mockEntries = [
  makeCatalogEntry({ name: "test-skill", displayName: "Test Skill", description: "A test skill", path: "knowledge/test-skill", version: "1.0.0" }),
];
...
describe("handleRequest", () => {
  ...
  test("GET /api/artifact/nonexistent/content returns 404", async () => {
    const req = new Request("http://localhost/api/artifact/nonexistent/content");
    const res = await handleRequest(req, mockEntries, mockHtml);
...
describe("browse SPA format display", () => {
  const entriesWithFormat = [
    makeCatalogEntry({ name: "format-skill", displayName: "Format Skill", description: "A skill with formatByHarness", keywords: ["format"], harnesses: ["kiro", "cursor"], path: "knowledge/format-skill", formatByHarness: { kiro: "power", cursor: "rule" } }),
    makeCatalogEntry({ name: "agent-skill", displayName: "Agent Skill", description: "A skill with agent format", keywords: ["agent"], harnesses: ["copilot", "qdeveloper"], path: "knowledge/agent-skill", formatByHarness: { copilot: "agent", qdeveloper: "agent" } }),
  ];

  const entryWithoutFormat = [
    makeCatalogEntry({ name: "legacy-skill", displayName: "Legacy Skill", description: "A skill without formatByHarness", keywords: ["legacy"], harnesses: ["kiro", "cursor"], path: "knowledge/legacy-skill" }),
  ];
  ...
  test("catalog API returns entries with formatByHarness", async () => {
    ...
    expect(body[1].formatByHarness).toEqual({ copilot: "agent", qdeveloper: "agent" });
  });
...
describe("browse server integration", () => {
  ...
  const testEntries = [
    makeCatalogEntry({ name: "integration-test", displayName: "Integration Test", description: "Test artifact", path: "knowledge/integration-test" }),
  ];
...
describe("browse SPA format integration", () => {
  ...
  const formatEntries = [
    makeCatalogEntry({ name: "power-artifact", displayName: "Power Artifact", description: "A kiro power", keywords: ["power"], harnesses: ["kiro", "cursor"], path: "knowledge/power-artifact", formatByHarness: { kiro: "power", cursor: "rule" } }),
    makeCatalogEntry({ name: "legacy-artifact", displayName: "Legacy Artifact", description: "No formatByHarness", keywords: ["legacy"], harnesses: ["kiro"], path: "knowledge/legacy-artifact" }),
  ];
  ...
  test("catalog API serves formatByHarness data for format entries", async () => {
    const res = await fetch(`${baseUrl}/api/catalog`);
    const body = await res.json();
    const powerEntry = body.find((e: { name: string }) => e.name === "power-artifact");
    expect(powerEntry.formatByHarness).toEqual({ kiro: "power", cursor: "rule" });
  });

  test("catalog API serves entries without formatByHarness for backward compat", async () => {
    const res = await fetch(`${baseUrl}/api/catalog`);
    const body = await res.json();
    const legacyEntry = body.find((e: { name: string }) => e.name === "legacy-artifact");
