...
async function writeArtifact(
  ...
  if (config.categories) {
    lines.push(`categories: [${config.categories.map((c) => `"${c}"`).join(", ")}]`);
  }
  if (config.ecosystem) {
    lines.push(`ecosystem: [${config.ecosystem.map((e) => `"${e}"`).join(", ")}]`);
  }
  if (config.depends) {
    lines.push(`depends: [${config.depends.map((d) => `"${d}"`).join(", ")}]`);
  }
  if (config.enhances) {
    lines.push(`enhances: [${config.enhances.map((e) => `"${e}"`).join(", ")}]`);
  }
...
describe("Catalog generation", () => {
  ...
  test("catalog contains exactly one entry per artifact with no duplicates", async () => {
    await writeArtifact(knowledgeDir, { name: "alpha-skill", description: "Alpha" });
    await writeArtifact(knowledgeDir, { name: "beta-skill", description: "Beta" });
    await writeArtifact(knowledgeDir, { name: "gamma-skill", description: "Gamma" });
  ...
  test("catalog JSON is valid UTF-8 and parseable", async () => {
    await writeArtifact(knowledgeDir, { name: "test-skill", description: "A test skill" });
    ...
    const expectedOrder = [
      ...
      "author", "version", "harnesses", "type", "path", "evals",
      "categories", "ecosystem", "depends", "enhances", "formatByHarness",
      "maturity", "model-assumptions", "collections",
    ];
