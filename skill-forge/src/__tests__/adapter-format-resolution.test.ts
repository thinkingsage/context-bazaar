...
function makeFrontmatter(overrides: Partial<Frontmatter> = {}): Frontmatter {
  return {
    ...
    author: "tester",
    version: "1.0.0",
    harnesses: ["kiro", "claude-code", "copilot", "cursor", "windsurf", "cline", "qdeveloper"],
    type: "skill",
    inclusion: "always",
    categories: [],
    ecosystem: [],
...
describe("kiroAdapter format resolution", () => {
  test("produces power output when format is 'power'", () => {
    ...
    const steeringFile = result.files.find((f) => f.relativePath === "steering/my-power.md");
  ...
  test("backward compat: power: true still produces power output with deprecation warning", () => {
    ...
    const steeringFile = result.files.find((f) => f.relativePath === "steering/legacy-power.md");
    expect(steeringFile).toBeDefined();

    // Should have deprecation warning
    const deprecationWarning = result.warnings.find((w) => w.message.includes("deprecated"));
...
describe("qdeveloperAdapter format resolution", () => {
  test("produces .q/agents/ output when format is 'agent'", () => {
    ...
    const agentFile = result.files.find((f) => f.relativePath === ".q/agents/test-artifact.md");
  ...
  test("still produces .q/agents/ from workflows when format is 'rule'", () => {
    ...
    const agentFile = result.files.find((f) => f.relativePath === ".q/agents/deploy.md");
    expect(agentFile).toBeDefined();
  });
});
