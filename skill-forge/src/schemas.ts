...
export const AssetTypeSchema = z.enum([
  "skill", "power", "rule",
  "workflow", "agent", "prompt", "template", "reference-pack",
]);
...
export const MaturitySchema = z.enum(["experimental", "beta", "stable", "deprecated"]);
export type Maturity = z.infer<typeof MaturitySchema>;

export const TrustLaneSchema = z.enum(["official", "partner", "community", "experimental"]);
...
export const CollectionSchema = z.object({
  name: z.string().min(1).regex(/^[a-z0-9]+(-[a-z0-9]+)*$/, "Collection name must be kebab-case"),
...
export const FrontmatterSchema = z.object({
  ...
  collections: z.array(z.string().min(1).regex(/^[a-z0-9]+(-[a-z0-9]+)*$/)).default([]),
  "inherit-hooks": z.boolean().default(false),
}).passthrough().superRefine((data, ctx) => {
  const harnessConfig = data["harness-config"] as Record<string, Record<string, unknown>> | undefined;
  if (!harnessConfig || typeof harnessConfig !== "object") return;

  for (const [harness, config] of Object.entries(harnessConfig)) {
    if (!config || typeof config !== "object" || !("format" in config)) continue;
