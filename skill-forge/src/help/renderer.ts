...
export function renderRootHelp(
  ...
  const allNames = [
    ...
    ...(commands.flatMap((cmd) => cmd.subcommands?.map((s) => `  ${s.name}`) ?? [])),
  ];
...
export function renderCommandHelp(
  ...
  const augmentedOptions = options.map((opt) => {
    if (
      meta?.showHarnessList &&
      opt.flags.includes("--harness")
    ) {
  ...
  if (subcommands && subcommands.length > 0) {
    ...
    for (const sub of subcommands) {
      lines.push(`${pad}${pad}${cyan(sub.name.padEnd(nameColWidth))}${sub.description}`);
    }
    lines.push("");
  }
