...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (request.params.name) {
    case "catalog_list": {
      ...
      if (args.collection) {
        entries = entries.filter((e) =>
          Array.isArray(e.collections) && e.collections.includes(args.collection),
        );
      }
      if (args.type) {
    ...
    case "artifact_content": {
      ...
      if (!entry) {
        return {
          content: [{ type: "text" as const, text: `Artifact '${args.name}' not found in catalog.` }],
          isError: true,
        };
      }
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Artifact found in catalog but source file not available at ${contentPath}.` }],
          isError: true,
        };
      }
    ...
    case "collection_list": {
      ...
      for (const entry of catalog) {
        for (const col of (Array.isArray(entry.collections) ? entry.collections : []) as string[]) {
