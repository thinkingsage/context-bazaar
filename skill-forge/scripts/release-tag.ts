...
function exec(cmd: string, opts?: { cwd?: string }) {
  return execSync(cmd, { encoding: "utf-8", cwd: opts?.cwd ?? projectRoot }).trim();
}
...
function writePkg(pkg: Record<string, unknown>) {
  writeFileSync(resolve(projectRoot, "package.json"), JSON.stringify(pkg, null, 2) + "\n");
}
...
function bumpVersion(current: string, bump: string): string {
  ...
  switch (bump) {
    ...
    default:
      console.error(`Unknown bump type: "${bump}". Use major, minor, patch, or an explicit version.`);
      process.exit(1);
  }
}
...
if (!bump) {
  ...
  const map: Record<string, string> = { "1": "patch", "2": "minor", "3": "major" };
  bump = map[input];
  if (!bump) {
...
if (dryRun) {
  console.log("[dry-run] Would update package.json, compile changelog, commit, and tag.");
  process.exit(0);
}
...
const unexpectedChanges = status
  .split("\n")
  .filter((l) => l.trim() && !l.includes("changes/") && !l.includes("package.json"));
if (unexpectedChanges.length > 0) {
  console.error("Error: Working tree has uncommitted changes outside changes/ and package.json:");
