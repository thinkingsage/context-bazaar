...
export class CachedEmbeddingProvider implements EmbeddingProvider {
  ...
  private getFromSqlite(hash: string): number[] | null {
    ...
    try {
      ...
      ).get(hash) as { embedding: Uint8Array | Buffer } | null;

      if (row) {
        // Bun's SQLite returns Uint8Array for BLOB columns; use TextDecoder for safe conversion
        const raw = row.embedding instanceof Uint8Array
          ? new TextDecoder().decode(row.embedding)
          : row.embedding.toString();
  ...
  private async getFromSolr(hash: string): Promise<number[] | null> {
    ...
    try {
      // findByContentHash is added in task 21.3 — check if method exists
      if (typeof (this.solrClient as any).findByContentHash !== "function") {
        this.stats.solr.misses++;
        return null;
      }

      const doc = await (this.solrClient as any).findByContentHash(hash);
      if (doc?.vector) {
