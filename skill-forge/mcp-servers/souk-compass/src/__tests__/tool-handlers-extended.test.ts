...
describe("compass_search extended", () => {
  ...
  test("mode=vector uses kNN search (embed called, no queryText)", async () => {
    ...
    const ctx = makeCtx({
      embeddingProvider: makeMockEmbeddingProvider({
        embed: async () => { embedCalled = true; return Array(1024).fill(0.2); },
      }),
      solrClient: makeMockSolrClient({
    ...
    await handleCompassSearch(
      { query: "git workflow", topK: 5, scope: "artifacts", mode: "vector", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("mode=keyword skips embed call", async () => {
    ...
    await handleCompassSearch(
      { query: "git workflow", topK: 5, scope: "artifacts", mode: "keyword", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("mode=hybrid calls embed and passes combined query params", async () => {
    ...
    const ctx = makeCtx({
      embeddingProvider: makeMockEmbeddingProvider({
        embed: async () => { embedCalled = true; return Array(1024).fill(0.3); },
      }),
      solrClient: makeMockSolrClient({
        search: async (_emb, _topK, opts) => {
          capturedOpts = opts as Record<string, unknown>;
    ...
    await handleCompassSearch(
      { query: "git workflow", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 0.7, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("hybridWeight=0.0 passes pure keyword weight", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 0.0, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("hybridWeight=1.0 passes pure vector weight", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 1.0, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("snippets from highlighting in keyword mode", async () => {
    ...
    const result = await handleCompassSearch(
      { query: "git", topK: 5, scope: "artifacts", mode: "keyword", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("snippets from highlighting in hybrid mode", async () => {
    ...
    const result = await handleCompassSearch(
      { query: "git", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
    const data = parseResult(result);
    const results = data.results as Array<Record<string, unknown>>;
  ...
  test("snippets from text truncation in vector mode", async () => {
    ...
    const result = await handleCompassSearch(
      { query: "git", topK: 5, scope: "artifacts", mode: "vector", hybridWeight: 0.5, snippetLength: 100, includeContent: false },
      ctx,
    );
    const data = parseResult(result);
    const results = data.results as Array<Record<string, unknown>>;
  ...
  test("snippetLength parameter controls truncation length", async () => {
    ...
    const result = await handleCompassSearch(
      { query: "git", topK: 5, scope: "artifacts", mode: "vector", hybridWeight: 0.5, snippetLength: 50, includeContent: false },
      ctx,
    );
    const data = parseResult(result);
    const results = data.results as Array<Record<string, unknown>>;
  ...
  test("minScore tool param takes precedence over config defaultMinScore", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, minScore: 0.9, scope: "artifacts", mode: "vector", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("config defaultMinScore used when tool param not set", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, scope: "artifacts", mode: "vector", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("snippetLength passed to search options", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 300, includeContent: false },
      ctx,
    );
...
describe("compass_index_artifacts extended", () => {
  ...
  test("chunked=true produces chunk documents with correct IDs", async () => {
    ...
    const upsertCalls: Array<{ docId: string; metadata: Record<string, string> }> = [];
    const ctx = makeCtx({
      solrClient: makeMockSolrClient({
    ...
    const result = await handleCompassIndexArtifacts({ name: "commit-craft", chunked: true }, ctx);
    const data = parseResult(result);
  ...
  test("chunked=true chunk documents have parent_artifact metadata", async () => {
    ...
    await handleCompassIndexArtifacts({ name: "commit-craft", chunked: true }, ctx);
...
describe("compass_reindex handler", () => {
  ...
  test("detects unchanged artifacts (same version and hash)", async () => {
    ...
    const text1 = buildEmbeddingText("Commit Craft", "A skill for crafting git commits", "# commit-craft\n\nThis is the body of commit-craft.");
    const text2 = buildEmbeddingText("Code Review", "A skill for code reviews", "# code-review\n\nThis is the body of code-review.");

    mockExistingDocs([
      { id: "commit-craft", version: "1.0.0", content_hash: contentHash(text1) },
  ...
  test("detects hash change → updated", async () => {
    ...
    mockExistingDocs([
      { id: "commit-craft", version: "1.0.0", content_hash: "stale-hash-value" },
    ]);
  ...
  test("detects removed artifacts (in Solr but not catalog)", async () => {
    ...
    const ctx = makeCtx({
      solrClient: makeMockSolrClient({
        delete: async (docId) => { deletedIds.push(docId); },
        commit: async () => {},
      }),
    });
  ...
  test("returns correct summary counts", async () => {
    ...
    expect((data.added as number) + (data.updated as number) + (data.unchanged as number)).toBe(2);
  });
});
...
describe("compass_status extended", () => {
  ...
  test("returns cache stats per tier with all three tiers", async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: { numFound: 10 } }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: { numFound: 5 } }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: { numFound: 2 } }), { status: 200 }));
  ...
  test("no cache stats when provider lacks getStats", async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: { numFound: 0 } }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: { numFound: 0 } }), { status: 200 }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ response: { numFound: 0 } }), { status: 200 }));
