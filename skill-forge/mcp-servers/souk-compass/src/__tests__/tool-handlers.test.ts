...
import type { SoukVectorClient, SolrSearchResponse } from "../solr-client.js";
...
function makeMockEmbeddingProvider(overrides?: Partial<EmbeddingProvider>): EmbeddingProvider {
  return {
    name: "mock-provider",
    dimensions: 1024,
...
function makeMockSolrClient(overrides?: Partial<SoukVectorClient>): SoukVectorClient {
  return {
    upsert: async () => {},
    search: async () => ({
...
describe("compass_setup handler", () => {
  ...
  test("check action returns structured status", async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 42 } }), { status: 200 }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 5 } }), { status: 200 }),
      );
  ...
  test("Docker not available returns helpful message for start action", async () => {
    ...
    const result = await handleCompassSetup({ action: "create_collections" }, ctx);
    const data = parseResult(result);
  ...
  test("create_collections calls Solr Collections API for both collections", async () => {
    ...
    const collections = data.collections as Array<{ name: string; created: boolean }>;
  ...
  test("create_collections handles already-existing collection", async () => {
    ...
    fetchSpy.mockResolvedValueOnce(
      new Response(JSON.stringify({ error: "collection already exists" }), { status: 400 }),
    );
    ...
    const result = await handleCompassSetup({ action: "create_collections" }, ctx);
    const data = parseResult(result);
    const collections = data.collections as Array<{ name: string; created: boolean; error?: string }>;
...
mock.module("../catalog-reader.js", () => ({
  ...
  readArtifactContent: async (_pluginRoot: string, entry: { name: string }) => ({
    frontmatter: {},
...
describe("compass_index_artifacts handler", () => {
  async function importHandler() {
    return (await import("../tools/compass-index.js")).handleCompassIndexArtifacts;
  }
  ...
  test("indexes a single artifact by name", async () => {
    ...
    const ctx = makeCtx({
      solrClient: makeMockSolrClient({
        upsert: async (docId) => { upsertCalls.push({ docId }); },
      }),
    });

    const result = await handleCompassIndexArtifacts({ name: "commit-craft" }, ctx);
    const data = parseResult(result);
  ...
  test("returns error when artifact not found in catalog", async () => {
    ...
    const result = await handleCompassIndexArtifacts({ name: "nonexistent" }, ctx);
    const data = parseResult(result);
...
describe("compass_search handler", () => {
  ...
  function makeSolrResponse(docs: Record<string, unknown>[]): SolrSearchResponse {
    return {
  ...
  test("basic query returns formatted results", async () => {
    ...
    const result = await handleCompassSearch(
      { query: "git workflow", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
    const data = parseResult(result);
  ...
  test("topK is passed to search client", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 10, scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("type filter is applied", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, type: "skill", scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("collection filter is applied", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, collection: "kiro-official", scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("maturity filter is applied", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, maturity: "stable", scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("combined filters use AND", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, type: "skill", maturity: "stable", scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("scope=documents searches user collection", async () => {
    ...
    const ctx = makeCtx({
      solrClient: makeMockSolrClient({
        search: async () => { artifactSearched = true; return makeSolrResponse([]); },
      }),
      userSolrClient: makeMockSolrClient({
        search: async () => { userSearched = true; return makeSolrResponse([]); },
      }),
    });

    await handleCompassSearch(
      { query: "test", topK: 5, scope: "documents", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("scope=all searches both collections", async () => {
    ...
    await handleCompassSearch(
      { query: "test", topK: 5, scope: "all", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
  ...
  test("no results returns message", async () => {
    ...
    const result = await handleCompassSearch(
      { query: "nonexistent", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
    const data = parseResult(result);

    expect(data.message).toContain("No semantically similar artifacts were found");
  ...
  test("Solr unreachable returns error message", async () => {
    ...
    const result = await handleCompassSearch(
      { query: "test", topK: 5, scope: "artifacts", mode: "hybrid", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
    const data = parseResult(result);
  ...
  test("minScore triggers searchByThreshold", async () => {
    ...
    const ctx = makeCtx({
      solrClient: makeMockSolrClient({
        search: async () => { regularCalled = true; return makeSolrResponse([]); },
        searchByThreshold: async () => { thresholdCalled = true; return makeSolrResponse([]); },
      }),
    });

    await handleCompassSearch(
      { query: "test", topK: 5, minScore: 0.8, scope: "artifacts", mode: "vector", hybridWeight: 0.5, snippetLength: 200, includeContent: false },
      ctx,
    );
...
describe("compass_index_document handler", () => {
  async function importHandler() {
    return (await import("../tools/compass-index-doc.js")).handleCompassIndexDocument;
  }
  ...
  test("basic document indexing", async () => {
    ...
    const ctx = makeCtx({
      userSolrClient: makeMockSolrClient({
        upsert: async (docId) => { upsertDocId = docId; },
      }),
    });
  ...
  test("indexes with custom collection parameter", async () => {
    ...
    const result = await handleCompassIndexDocument(
      { id: "doc-2", text: "Custom collection doc", collection: "my-custom-collection" },
      ctx,
    );
    const data = parseResult(result);
  ...
  test("indexes with metadata", async () => {
    ...
    await handleCompassIndexDocument(
      { id: "doc-3", text: "With metadata", metadata: { project: "test", lang: "ts" } },
      ctx,
    );
...
describe("compass_status handler", () => {
  ...
  test("returns document counts per collection", async () => {
    ...
    fetchSpy
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 7 } }), { status: 200 }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 3 } }), { status: 200 }),
      );
    ...
    expect(data).toHaveProperty("collections");
    const collections = data.collections as Array<{ name: string; docCount: number | null }>;
  ...
  test("handles Solr unreachable gracefully", async () => {
    ...
    expect(data).toHaveProperty("collections");
    const collections = data.collections as Array<{ name: string; docCount: number | null; error?: string }>;
  ...
  test("includes cache stats when provider has getStats", async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 0 } }), { status: 200 }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 0 } }), { status: 200 }),
      )
...
describe("compass_health handler", () => {
  ...
  test("returns connectivity status when Solr is reachable", async () => {
    ...
    fetchSpy
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 10 } }), { status: 200 }),
      )
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ response: { numFound: 5 } }), { status: 200 }),
      );
    ...
    const collections = data.collections as Array<{ name: string; exists: boolean }>;
    expect(collections.length).toBe(2);
