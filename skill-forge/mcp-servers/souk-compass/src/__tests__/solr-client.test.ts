...
function errorResponse(status: number, body?: unknown): Response {
  return new Response(JSON.stringify(body ?? { error: { msg: "Solr error" } }), {
    status,
    headers: { "Content-Type": "application/json" },
  });
}
...
describe("SoukVectorClient", () => {
  ...
  describe("upsert", () => {
    ...
    test("commit=false defers commit", async () => {
      ...
      expect(url).toBe(
        `${BASE_URL}/solr/${COLLECTION}/update/json/docs`,
      );
    });
    ...
    test("throws SoukCompassError on HTTP error", async () => {
      ...
      await expect(
        client.upsert("doc-1", "text", [1], {}),
      ).rejects.toThrow(SoukCompassError);

      try {
        fetchSpy.mockResolvedValueOnce(errorResponse(500, { error: { msg: "bad request" } }));
  ...
  describe("search", () => {
    ...
    test("constructs correct kNN query in vector mode (default)", async () => {
      ...
      expect(q).toBe(`{!knn f=vector topK=5 earlyTermination=true}${JSON.stringify(embedding)}`);
    });
    ...
    test("keyword mode uses BM25 text query without embedding", async () => {
      ...
      await client.search(null, 10, { mode: "keyword", queryText: "git workflow" });
  ...
  describe("URL normalization", () => {
    test("strips trailing slash from base URL", async () => {
      const clientWithSlash = new SoukVectorClient("http://localhost:8983/", COLLECTION);
      fetchSpy.mockResolvedValueOnce(okJson({ responseHeader: { status: 0 } }));

      await clientWithSlash.commit();

      const [url] = fetchSpy.mock.calls[0] as [string];
      expect(url).toBe(`http://localhost:8983/solr/${COLLECTION}/update?commit=true`);
    });
  });
});
