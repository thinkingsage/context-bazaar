...
function makeMockProvider(overrides?: Partial<EmbeddingProvider>): EmbeddingProvider {
  return {
    name: "mock-provider",
    dimensions: 4,
    embed: async (_text: string) => [0.1, 0.2, 0.3, 0.4],
    batchEmbed: async (texts: string[]) => texts.map(() => [0.1, 0.2, 0.3, 0.4]),
    ...overrides,
  };
}
...
describe("CachedEmbeddingProvider", () => {
  ...
  describe("SQLite tier", () => {
    ...
    test("auto-creates DB file and directory", async () => {
      const nestedPath = join("/tmp", `souk-test-nested-${Date.now()}`, "sub", "cache.db");
      const provider = makeMockProvider();
    ...
    test("corrupted SQLite logs warning and skips tier", async () => {
      ...
      const errorMsg = consoleErrorSpy.mock.calls.find(
        (c) => typeof c[0] === "string" && c[0].includes("SQLite cache init failed"),
      );
  ...
  describe("batchEmbed", () => {
    ...
    test("writes batch misses to memory and SQLite", async () => {
      ...
      const embedSpy = { called: false };
      const provider2 = makeMockProvider({
        embed: async () => {
          embedSpy.called = true;
          return [99, 99, 99, 99];
        },
      });
