...
export async function handleCompassReindex(
  ...
  try {
    ...
    const solrMap = new Map<string, { version?: string; contentHash?: string }>();
    for (const doc of existingDocs) {
      const id = doc.id as string;
      solrMap.set(id, {
        version: typeof doc.version === "string" ? doc.version : undefined,
        contentHash: typeof doc.content_hash === "string" ? doc.content_hash : undefined,
      });
    }
