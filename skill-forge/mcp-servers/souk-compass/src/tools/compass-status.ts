...
export async function handleCompassStatus(
  ...
  for (const { name, client } of [
    { name: ctx.config.solrCollection, client: ctx.solrClient },
    { name: ctx.config.userCollection, client: ctx.userSolrClient },
  ]) {
    try {
      const url = `${ctx.config.solrUrl}/solr/${encodeURIComponent(name)}/select?q=*:*&rows=0&wt=json`;
      const response = await fetch(url);
      if (!response.ok) {
        collections.push({ name, docCount: null, error: `HTTP ${response.status}` });
        continue;
      }
  ...
  const cacheStats = "getStats" in ctx.embeddingProvider
    ? (ctx.embeddingProvider as any).getStats()
    : null;
