...
export class SoukVectorClient {
  ...
  constructor(baseUrl: string, collection: string, options?: SoukVectorClientOptions) {
   ...
   */
  async findByContentHash(contentHash: string): Promise<Record<string, unknown> | null> {
    try {
      const params = new URLSearchParams({
