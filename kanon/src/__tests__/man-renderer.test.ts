...
describe("kanon man command", () => {
	...
	test("does not list man or help as commands", async () => {
		...
		// COMMANDS runs until EXAMPLES, or SEE ALSO if no command has examples.
		const commandsStart = stdout.indexOf(".SH COMMANDS");
		const examplesStart = stdout.indexOf(".SH EXAMPLES");
		const commandsEnd =
			examplesStart !== -1 ? examplesStart : stdout.indexOf(".SH SEE ALSO");
		const commandsSection = stdout.slice(commandsStart, commandsEnd);
